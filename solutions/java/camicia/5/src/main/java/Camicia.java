...
public class Camicia {
    ...
    static CamiciaResult simulateGame(List<String> playerA, List<String> playerB) {
        ...
        List<List<Integer>> decks = List.of(normalizeDeck(playerA), normalizeDeck(playerB));
        ...
        while (true) {
            String state = String.join("|", decks.toString(), pile.toString(), Integer.toString(turn), Integer.toString(penalty));

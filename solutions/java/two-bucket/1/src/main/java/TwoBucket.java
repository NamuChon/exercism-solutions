    ...
    TwoBucket(int bucketOneCap, int bucketTwoCap, int desiredLiters, String startBucket) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.getBucketOneLiters() == desiredLiters) {
                results.add(new Result(node.getDepth(), "one", node.getBucketTwoLiters()));
                continue;
            } else if (node.getBucketTwoLiters() == desiredLiters) {
                results.add(new Result(node.getDepth(), "two", node.getBucketOneLiters()));
                continue;
            }
        ...
        result = results.stream()
                .min(Comparator.comparing(Result::getTotalMoves))
                .orElse(null);
    }

...
RANKINGS = (
    ...
    lambda ranks, suits: same(ranks, 4),
    lambda ranks, suits: same(ranks, 3, 2),
    lambda ranks, suits: same(suits),
    lambda ranks, suits: consecutive(ranks),
    lambda ranks, suits: same(ranks, 3),
    lambda ranks, suits: same(ranks, 2, 2),
    lambda ranks, suits: same(ranks, 2)
)

def same(objects, *counts: int) -> bool:
    if not counts: counts = [len(objects)]
    frequencies = list(Counter(objects).values())
    for count in counts:
        found = False
...
class Hand:
    ...
    def compare_to(self, other) -> int:
        ...
        kicker_result = [t[0] - t[1] for t in zip(self.kicker, other.kicker) if t[0] - t[1]]
        return kicker_result[0] if kicker_result else 0
